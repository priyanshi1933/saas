...
import { createPaymentForOrganization, upsertStripePayment, type PaymentInput } from "./payment.service";
import { createSubscriptionForOrganization, upsertStripeSubscription, type SubscriptionInput } from "./subscription.service";
...
const getMetadata = (metadata: Record<string, string> | undefined, key: string, fallback = "") => {
  return (metadata && metadata[key]) || fallback;
};

export const createStripePaymentSession = async ({
  organizationId,
  clientName,
  amount,
}: {
...
export const constructStripeEvent = (payload: Buffer, signature: string | undefined) => {
  if (isFakeStripe) {
    try {
...
const resolveSubscriptionPayload = async (
  ...
  const billingCycle = (item.price.recurring?.interval === "year" ? "yearly" : "monthly") as "monthly" | "yearly";
...
const createPaymentRecord = async ({
  ...
  await upsertStripePayment(paymentInput);
};

const createOrUpdateSubscriptionRecord = async ({
  organizationId,
  clientName,
  planName,
  amount,
  ...
  await upsertStripeSubscription(subscriptionInput);
};
...
export const handleStripeEvent = async (event: any) => {
  ...
  switch (event.type) {
    ...
    case "payment_intent.succeeded": {
      const paymentIntent = data as any;
    ...
    case "customer.subscription.updated": {
      ...
      const plan = subscription.items.data[0]?.price as any;
      ...
      const billingCycle = (plan?.recurring?.interval === "year" ? "yearly" : "monthly") as "monthly" | "yearly";
